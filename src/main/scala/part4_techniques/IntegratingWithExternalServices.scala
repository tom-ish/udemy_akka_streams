...
object IntegratingWithExternalServices extends App {
  ...
  // import system.dispatcher // not recommanded in practice for MapAsync
  ...
  val eventSource = Source(List(
    ...
    PagerEvent("FastData", "Illegal elements in the data pipeline", new Date),
  ...
  val alternativePagedEngineerEmail = infraEvents.mapAsync(4)(event => (pagerActor ? event).mapTo[String])
  alternativePagedEngineerEmail.to(pagedEmailsSink).run()
